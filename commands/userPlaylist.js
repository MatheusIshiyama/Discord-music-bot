...
exports.run = async (bot, message, args) => {
    ...
    if(!args) {
        if(!userReq.playlist) {
            return embedReply("User Playlist", `use \`${guildReq.prefix}up add\`, to add a playlist link`, message);
        } else {
            return embedReply("User Playlist", userReq.playlist, message)
        }
    } else if (args == 'add') {
        resultsEmbed.addField('Digite o link da playlist', 'digite \`cancel\` se quiser cancelar.');
        const reply = await message.channel.send(resultsEmbed);
        function filter(msg) {
            return msg.content.includes('cancel') || msg.content.includes('youtube.com/playlist');
        };
